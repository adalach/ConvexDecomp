...
if __name__ == "__main__":
    # Example polygon with one hole (same as before)
    ext_coords = [
        (-5.900000, 22.600000),
        (-6.450000, 23.200000),
        (-5.780000, 23.470000),
    ...
    poly_with_hole = Polygon(ext_coords, [int_coords])

    print("Single-hole split:")
    result_single = split_polygon_with_hole(poly_with_hole)
    print(f"  Number of resulting polygons: {len(result_single)}")
    for i, p in enumerate(result_single):
        print(f"  Polygon {i}: area = {p.area:.3f}, holes = {len(p.interiors)}")

    print("\nRecursive all-hole split:")
    result_all = split_polygon_all_holes(poly_with_hole)
    print(f"  Number of resulting polygons: {len(result_all)}")
    for i, p in enumerate(result_all):
        print(f"  Polygon {i}: area = {p.area:.3f}, holes = {len(p.interiors)}")
