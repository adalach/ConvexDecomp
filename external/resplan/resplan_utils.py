...
import numpy as np
# Optional deps: cv2, networkx (used only in specific functions)
try:
    import cv2  # type: ignore
except Exception:  # ImportError or others in headless envs
...
CATEGORY_COLORS: Dict[str, str] = {
    ...
    "storage": "#db822b",    # orange-brown
    "stair": "#c49a6c"       # light brown
}
    ...
    if isinstance(geom, (list, tuple)):
        ...
        for g in geom:
            if isinstance(g, Polygon) and not g.is_empty:
                ...
                parts.extend([p for p in g.geoms if isinstance(p, Polygon) and not p.is_empty])
        return parts
    return []
...
def _poly_to_mask(poly: Polygon, shape: Tuple[int, int], line_thickness: int = 0) -> np.ndarray:
    if cv2 is None:
        raise ImportError("OpenCV (cv2) is required for _poly_to_mask(). Install with: pip install opencv-python")
    h, w = shape
...
def geometry_to_mask(geom: Any,
    ...
    if cv2 is None:
        raise ImportError("OpenCV (cv2) is required for geometry_to_mask(). Install with: pip install opencv-python")
    h, w = shape
...
def plot_plan(plan: Dict[str, Any],
    ...
    if categories is None:
        categories = ["living","bedroom","bathroom","kitchen","door","window","wall","front_door","balcony", "storage", "stair"]
...
def plan_to_graph(plan: Dict[str, Any],
    ...
    Requires NetworkX. If not installed, install with: pip install networkx
    """
    if nx is None:
        raise ImportError("NetworkX (networkx) is required for plan_to_graph(). Install with: pip install networkx")
    plan = normalize_keys(plan)
    G = nx.Graph()
...
def plot_plan_and_graph(plan: Dict[str, Any],
    ...
    """
    if nx is None:
        raise ImportError("NetworkX (networkx) is required for plot_plan_and_graph(). Install with: pip install networkx")
